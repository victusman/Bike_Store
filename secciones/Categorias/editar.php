...
$stmt = $pdo->prepare('SELECT * FROM categoria WHERE category_id = :id');
$stmt->execute(['id'=>$id]);
$cat = $stmt->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $pdo->prepare('UPDATE categoria SET descripcion=:d WHERE category_id=:id');
...
<form method="post" class="row g-3">
    <div class="col-12">
        ...
        <input class="form-control" type="text" name="descripcion" value="<?php echo htmlspecialchars($cat['descripcion']); ?>" required>
    </div>
    <div class="col-12">
