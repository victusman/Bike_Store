<?php
/**
 * FACTURA PDF CON ENVÍO AUTOMÁTICO DE EMAIL
 * Versión mejorada que envía email después de generar el PDF
 */
...
if (MODO_DEMO_FACTURAS) {
    ...
    $items = $productos_demo;
    
} else {
    // MODO PRODUCCIÓN - obtener datos reales de la base de datos
    try {
        $stmt = $pdo->prepare("
...
// ============================================
// Guardar PDF y datos en sesión para envío posterior
if (isset($_SESSION['customer_id']) && !isset($_GET['no_email'])) {
    $pdfContent = $dompdf->output();
    
    // Datos para el email
    $_SESSION['factura_email_' . $order_id] = [
        ...
        'cliente_nombre' => trim(($pedido['first_name'] ?? '') . ' ' . ($pedido['last_name'] ?? '')),
        'timestamp' => time()
    ];
}
...
// Agregar JavaScript para envío automático de email después de cargar PDF
?>
<script>
// Enviar email automáticamente cuando el PDF se haya cargado
window.addEventListener('load', function() {
    setTimeout(function() {
        // Crear imagen invisible para trigger del email
        var img = new Image();
        img.onload = function() {
            console.log('✅ Email enviado automáticamente');
        };
        img.onerror = function() {
            console.log('⚠️ Email no enviado o error en envío');
        };
        img.src = 'procesar_email_factura.php?order_id=<?php echo $order_id; ?>&t=' + Date.now();
    }, 1000); // Esperar 1 segundo después de cargar el PDF
});
</script>
<?php
